...
export default function InstructorTimetableLayout({ initialTimetable, instructor }: InstructorTimetableLayoutProps) {
    ...
    // Collect all course IDs this student is enrolled in by matching instructor_id across courses
    const studentCourseIds = useMemo(() => {
        const ids = new Set<number>()
        timetable.forEach(item => {
            const course: Course = item.course
            const isEnrolled = course.instructors.some(s => s.instructor_id === instructor.instructor_id)
            ...
            <div className="relative overflow-hidden rounded-2xl border border-slate-100 bg-gradient-to-r from-indigo-600 to-violet-800 p-5 md:p-6 shadow-xl">
                ...
                <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-[10px] font-bold uppercase tracking-widest text-indigo-300 mb-0.5">Instructor Schedule</p>
                            <h1 className="text-base md:text-xl font-extrabold text-white leading-tight">
                                ENG/{instructor.name?.toUpperCase() ?? "Unknown Instructor"}
                            </h1>
                            <div className="mt-1 flex flex-wrap items-center gap-2">
                                ...
                                {instructor.user_id && (
                                    <span className="flex items-center gap-1 rounded-md bg-indigo-500/20 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-indigo-300">
                                        <Layers className="h-2.5 w-2.5" /> UID {instructor.user_id}
                                    </span>
                                )}
                            </div>
                        </div>
