...
export function RoomsLayout({
  ...
  const handleOpenDialog = (item?: Room) => {
    if (item) {
      setEditingItem(item)
  ...
  const handleSubmit = () => {
    if (editingItem) {
      onEdit({ ...editingItem, ...formData } as Room)
    } else {
      onAdd(formData as Partial<Room>)
    }
  ...
  const getValue = (item: Room, key: string): unknown => {
