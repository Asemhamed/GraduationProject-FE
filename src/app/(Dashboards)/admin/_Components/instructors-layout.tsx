...
interface InstructorsLayoutProps {
  ...
  onAdd: (item: Partial<Instructor>) => void
...
export function InstructorsLayout({
  title,
  description,
  icon,
  columns,
  ...
  const handleOpenDialog = (item?: Instructor) => {
    if (item) {
      setEditingItem(item)
  ...
  const handleSubmit = () => {
    if (editingItem) {
      onEdit({ ...editingItem, ...formData } as Instructor)
    } else {
      onAdd(formData as Partial<Instructor>)
    }
  ...
  const getValue = (item: Instructor, key: string): unknown => {
