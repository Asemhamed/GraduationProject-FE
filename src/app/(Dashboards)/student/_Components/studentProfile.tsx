...
import { User, GraduationCap, Clock, Building2, MapPin } from "lucide-react"
import { AdminResponse } from "@/Types/AdminTypes"

export default function StudentProfile({ profile }: { profile: any }) {
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="relative -mt-10 flex items-center gap-4">
            ...
            <div className="pt-10">
              <div className="flex items-center gap-2">
                <h1 className="text-xl font-bold text-slate-900">{profile.full_name.toUpperCase()}</h1>
                <GraduationCap className="text-indigo-500 h-5 w-5" />
              </div>
              <p className="text-xs text-slate-500 font-medium">Student ID: {profile.user_id}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <InfoCard icon={<Clock className="text-indigo-500" size={18} />} label="Active Semester" value={profile.semester} />
      </div>
    </div>
  )
}
